...
- [x] Smart Capture: "team standup every monday at 9am for 30 mins" parses correctly — title, time, duration extracted
- [x] Smart Capture: falls back to plain task creation if AI call fails
- [x] Smart Capture: never auto-saves — always requires user confirmation
- [x] Task Breakdown: returns 3–7 subtasks for a vague task title
- [x] Task Breakdown: "Add selected" creates only checked subtasks
- [x] Task Breakdown: button hidden when task already has subtasks
- [x] Daily Planner: proposed schedule respects work hours from settings
- [x] Daily Planner: Q1 tasks appear earlier in the schedule than Q4 tasks
- [x] Daily Planner: already-scheduled tasks are not moved
- [x] All three: `OPENROUTER_API_KEY` never appears in client bundle (`grep -r OPENROUTER_API_KEY .next/` returns nothing)
- [x] All three: invalid JSON from model is caught and returns error toast
- [x] AI toggle in settings hides all ✨ buttons when disabled
